...
function updateBadges() {
  if (elements.valDrive) elements.valDrive.textContent = Number(state.drive).toFixed(2);
  if (elements.valTurn) elements.valTurn.textContent = Number(state.turn).toFixed(2);
  if (elements.valLight) elements.valLight.textContent = Math.round((state.flashLevel / 255) * 100) + "%";
}
...
const lockControls = Array.from(elements.buttonZone.querySelectorAll("[data-dir='FORWARD'], [data-dir='BACKWARD'], [data-dir='STOP']"));
const holdControls = Array.from(elements.buttonZone.querySelectorAll("[data-dir='LEFT'], [data-dir='RIGHT']"));

let lockIntervalId = null;

function handleLockPress(dir) {
  if (state.mode !== "button") return;
  if (lockIntervalId) { clearInterval(lockIntervalId); lockIntervalId = null; }
